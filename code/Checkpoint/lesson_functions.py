...
class detected_vehicles():
    ...
    def process_img_heatmap(self, img):
        ...
        self.heatmap = cv2.filter2D(self.heatmap,-1,kernel)
        
        self.heatmap = apply_threshold(self.heatmap, 2.2)
        labels = label(self.heatmap)
        print(labels[1], 'cars found')
        draw_img = np.copy(img)
        draw_img = draw_labeled_bboxes(draw_img, labels)
        return draw_img #heatmap_for_label


# Draw bounded boxes on car
...
def apply_threshold(heatmap, threshold):
    temp_heatmap = np.copy(heatmap)
    # Zero out pixels below the threshold
    temp_heatmap[temp_heatmap <= threshold] = 0
